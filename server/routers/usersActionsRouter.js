...
//the below errors come from performing {updateActionAllowd}
// catch(e){
//      console.error(e); 
...
//     return res.status(500).json({ error: 'server error' , msg:'server'});
// }

catch (e) {
  console.error('usersActionsRouter error:', e);
  if (e.code === 'DAILY_LIMIT') return res.status(403).json({ error: e.message });
  if (e.code === 'NOT FOUND')   return res.status(404).json({ error: e.message });
